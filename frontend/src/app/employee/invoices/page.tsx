...
export default function EmployeeInvoicesCompaniesPage() {
  ...
  const invoices = (data?.data ?? []) as EmployeeInvoice[];
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <Button
          size="sm"
          className="bg-jojo-orange hover:bg-jojo-orange/90 text-black font-medium text-xs px-3 py-1.5 h-8"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {companies.map((c) => (
            <Card
              ...
              onClick={() =>
                router.push(
                  `/employee/invoices/${encodeURIComponent(c.company)}`,
                )
              }
              onKeyDown={(e) => {
